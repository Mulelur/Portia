...
import { Expertise, Hobbies } from "../../components";
...
export default function ExpertiseContainer() {
  return (
    <Expertise>
      <Expertise.Content>
        <Expertise.Text>
          My name is Tintswalo Portia Mabunda. I am a first year student at VUT
        ...
        {MMData.map((item) => {
          return (
            <>
              ...
              <Expertise.MMContent>
                {item.task.map((item) => {
                  return <Expertise.MMText>{item}</Expertise.MMText>;
                })}
              </Expertise.MMContent>
            </>
          );
